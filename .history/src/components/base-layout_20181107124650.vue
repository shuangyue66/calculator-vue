<template>
    <div class="containter-wrap">
        <div class="show-calculate-wrap">{{ calculate }}</div>
        <div class="show-num-wrap">{{ digital }}</div>
        <div class="btn-wrap">
            <btn v-for="(item, index) in btnArr" :key="index + item" :itemData="item" @test="testClick"></btn>
        </div>
    </div>
</template>

<script>
    import btn from './btn.vue'

    export default {
        name: 'baseLayout',
        components: {
            btn
        },
        data: () => {
            return {
                btnArr: [
                    {
                        content: 'c',
                        className: 'gay',                 
                    },
                    {
                        content: '+/-',
                        className: 'gay',
                    },
                    {
                        content: '%',
                        className: 'gay',
                    },
                    {
                        content: '÷',
                        className: 'orange',
                    },
                    {
                        content: '7',
                        className: 'brown',
                    },
                    {
                        content: '8',
                        className: 'brown',
                    },
                    {
                        content: '9',
                        className: 'brown',
                    },
                    {
                        content: '×',
                        className: 'orange',
                    },
                    {
                        content: '4',
                        className: 'brown',
                    },
                    {
                        content: '5',
                        className: 'brown',
                    },
                    {
                        content: '6',
                        className: 'brown',
                    },
                    {
                        content: '-',
                        className: 'orange',
                    },
                    {
                        content: '1',
                        className: 'brown',
                    },
                    {
                        content: '2',
                        className: 'brown',
                    },
                    {
                        content: '3',
                        className: 'brown',
                    },
                    {
                        content: '+',
                        className: 'orange',
                    },
                    {
                        content: '0',
                        className: 'n0',
                    },
                    {
                        content: '.',
                        className: 'brown',
                    },
                    {
                        content: '=',
                        className: 'orange',
                    },


                ],
                calculate: '',
                digital: 0,
                firstNum: '',
                secondNum: '',
                operation: '',
                nowClick: ''
            }          
        },
        methods: {
            testClick (op) {
                // 当是c的时候，直接清空所有的，变为初始值，所以直接就可以return不需要再走过多的逻辑
                if (op.content === 'c') {
                    this.calculate = ''
                    this.digital = 0,
                    this.firstNum = ''
                    this.secondNum = ''
                    this.lastClick = ''
                    this.operation = ''
                    return
                }
                    var nowClick = ''; 
                    switch(op.content) {
                        case '0': case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '8': case '9':
                            if (this.nowClick === '=') {
                                this.firstNum = ''
                                this.operation = ''
                            }
                            if (this.operation) {
                                this.secondNum += op.content
                            } else {
                                this.firstNum += op.content
                            }
                                this.nowClick = 'Num'
                        break;
                        case '÷': case '×': case '+': case '-':
                            
                            this.operation = op.content
                            this.nowClick = 'operation'
                        break;
                        case '=':
                            this.nowClick = '='
                        break;
                    }   
                        

                        if (this.nowClick === 'operation') {
                            if (this.secondNum) {
                                switch (this.operation){
                                    case '+':
                                        this.digital = Number(this.firstNum) + Number(this.secondNum) 
                                        break;
                                    case '-':
                                        this.digital = Number(this.firstNum) - Number(this.secondNum) 
                                        break;
                                    case '×':
                                        this.digital = Number(this.firstNum) * Number(this.secondNum)
                                        break;
                                    case '÷':
                                        this.digital = Number(this.firstNum) / Number(this.secondNum)
                                        break;
                                }
                                
                                this.calculate = this.calculate + this.secondNum + this.operation
                                this.firstNum = this.digital
                                this.secondNum = '' 
                                
                            } else {
                                if (this.lastClick === 'Num') {
                                    this.calculate = this.firstNum + this.operation
                                } else {
                                    this.calculate = this.calculate.slice(0, this.calculate.length - 1) +  this.operation          
                                }
                            }
                        } else if(op.content === '=') {
                            switch (this.operation){
                                case '+':
                                    this.digital = Number(this.firstNum) + Number(this.secondNum) 
                                break;
                                case '-':
                                    this.digital = Number(this.firstNum) - Number(this.secondNum) 
                                break;
                                case '×':
                                    this.digital = Number(this.firstNum) * Number(this.secondNum)
                                break;
                                case '÷':
                                    this.digital = Number(this.firstNum) / Number(this.secondNum)
                                break;
                            }
                            this.calculate = ''
                            this.firstNum = this.digital
                            this.secondNum = ''

                        }else {
                            if (this.operation) {
                                this.digital = this.secondNum        
                            } else {
                                this.digital = this.firstNum
                            }
                        }   
                        this.lastClick =  this.nowClick
            }
        }
    }

    // outer:
                // switch (op.value) {
                //     case 'c':
                //         this.nowNum = '';
                //         this.first = '';
                //         this.calculate = '';
                //         this.nowcount = 0;
                //         this.nowthen = 0;
                //         this.shang = 0;
                //         this.xia = '';
                //         break;
                //     case '0': case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '8': case '9':
                //         if(this.shang === 0){
                //             this.first += op.value;
                //             this.nowNum = this.first;
                //         } else {
                //             this.xia += op.value;
                //             this.nowNum = this.xia;
                //         }
                //         break;
                //     case '÷': case '×': case '+': case '-': 
                //         this.calculate = this.nowNum + ' ' + op.value;  
                //         this.nowthen = op.value; 
                //         this.shang = this.nowNum;
                //         this.isEqual = false
                //         break;
                //     case '=':
                //         inner: 
                //         switch(this.nowthen){
                //             case '+':
                //                 this.nowNum = Number(this.shang) + Number(this.nowNum);
                //                 break inner;
                //             case '-':
                //                 this.nowNum = Number(this.shang) - Number(this.nowNum);
                //                 break inner;
                //             case '×':
                //                 this.nowNum = Number(this.shang) * Number(this.nowNum);
                //                 break inner;
                //             case '÷':
                //                 this.nowNum = Number(this.shang) / Number(this.nowNum);
                //                 break inner;
                //             case '=':
                            
                //                 break inner;
                //         }
                //         this.isEqual = true
                //         this.calculate = '';
                //     break outer;                 
                // }   
                        
                // if (op.value === 'c') {
                //     this.digital = 0
                // } else {
                //     this.digital = this.nowNum;
                // }
</script>

<style scoped>

.containter-wrap {
    width: 540px;
    height: 960px;
    background-color: black;
    margin: auto;
}

.show-calculate-wrap {
    color: gray;
    padding: 143px 25px 0 0;
    font-size: 40px;
    height: 40px;
    line-height: 40px;
    text-align: right;
}

.show-num-wrap {
    color: #fff;
    padding: 0px 25px 0 0;
    font-size: 88px;
    height: 88px;
    line-height: 88px;  
    text-align: right;
    margin: 0px 0px 40px 0px;
}
</style>